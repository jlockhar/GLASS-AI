...
author: "GLASS-AI version 0.9.5.1 ^[https://github.com/jlockhar/GLASS-AI]"
...
packages <- c(
  ...
  "viridis", # colorblind-friendly color palette
  "readxl", # read input xlsx files
  "knitr", # make tables
  "stringr", # easier string operations
  "janitor", # clean factor names in dataframes
  "cowplot", # simplify plotting of multiple ggplot2 objects in a single frame
...
import_individual_slide_tumors <- function(dir_path, image_info, image_id_column = "image_id") {
  ...
  matched_files <- tools::file_path_sans_ext(file_list) %in% provided_slide_ids
  ...
  for (file in file_list[-1]) {
    ...
    merged_tumor_areas <- rbind(merged_tumor_areas, slide_data)
  }
...
permissive_shapiro <- function(xdf, xformula, p.adj.method = "holm") {
  ...
  valid_groups <- group_tally %>% filter(n >= 3, n < 5000)
  ...
  if (nrow(valid_groups) > 0) {
    shapiro_result <- suppressMessages(
      xdf %>% semi_join(valid_groups, by = xvars) %>%
        group_by(across(all_of(xvars))) %>% shapiro_test(!!as.name(yvar))
    )
    shapiro_result <- suppressMessages(
      full_join(shapiro_result, valid_groups) %>%
        full_join(small_groups) %>%
        full_join(big_groups) %>%
        adjust_pvalue(method = p.adj.method) %>%
        relocate(p.adj, n, note, .after = p) %>%
        arrange(across(all_of(xvars)))
    )
  } else {
    shapiro_result <- suppressMessages(full_join(small_groups, big_groups) %>%
      adjust_pvalue(method = p.adj.method) %>%
      relocate(p.adj, n, note, .after = p) %>%
      arrange(across(all_of(xvars))))
  }
...
validate_groups <- function(xdf, xformula) {
  ...
  group_tally <- xdf %>%
    group_by(across(all_of(xvars))) %>%
    tally()
  valid_groups <- group_tally %>% filter(n >= 3)
  return(valid_groups)
}
...
#' Format a specific trace or type of trace in a plotly object.
#' 
#' @description
...
format_plotly_trace <- function(plotly_object, filter_type = NULL, filter_mode = NULL, filter_name = NULL, ...) {
  ...
  for (plotly_trace in plotly_object$x$data) {
    ...
    if ("type" %in% names(plotly_trace) &&
      (plotly_trace$type == filter_type || is.null(filter_type)) &&
      (plotly_trace$mode == filter_mode || is.null(filter_mode)) &&
      (plotly_trace$name == filter_name || is.null(filter_name))){
      found_trace <- TRUE
...
format_plotly_legend <- function(plotly_object, legend_group_variable, level_label_list, legend_label_variable = "label", make_multiline = F, split_delimiter = ":") {
  trace_index <- 0
  for (plotly_trace in plotly_object$x$data) {
    trace_index <- trace_index + 1
...
run_omnibus_test <- function(xdf, xformula, useParametric = "auto", useWelch = "auto", stats_summary = NULL) {
  ...
  valid_groups <- validate_groups(xdf, xformula)
  ...
  if (useParametric) {
    ...
    omni.test <- valid_xdf %>%
      ...
      group_by(across(all_of(xvars[-1]))) %>%
      # remove levels that don't have multiple groups
      filter(n_distinct(across(xvars[1])) > 1) %>%
...
dir_path <- "/Users/4470641/Desktop/GLASS-AI reports/4 group test/"
# path to slide info file
path_to_Slide_Info <- paste0(dir_path, "Slide IDs.xlsx")
# path to whole slide summary file
path_to_slide_areas <- paste0(dir_path, "Whole slide summary.xlsx")
# path to merged tumor area file or directory containing indivudal slide results
path_to_tumor_areas <- paste0(dir_path, "merged_areas.csv")
# define path to folder holding individual IHC stain subfolders labeled by stain
path_to_ihc_analysis_folder <- ""
# path to GLASS-AI logfile if you want to include in report
path_to_logfile <- ""
# define output location for data tables used to make each graph
path_to_graph_data <- paste0(dir_path, "Graph Data")


#### FILTERING####
# define optional tumor size threshold in sq. micron
tumor_size_threshold <- 0

#### GROUPING####
# define variables to use for grouping
full_grouping_vars <- c("genotype")
full_grouping_vars_labels <- c("Genotype")

# define order of groups at each level
...
full_grouping_order_labels <- list(genotype = c(
    "K",
    "dNK",
    "KP",
    "dNKP"
...
if (file.exists(path_to_tumor_areas) && !dir.exists(path_to_tumor_areas)) {
  ...
  Tumor_Areas <- import_individual_slide_tumors(path_to_tumor_areas, Slide_Info)
}
...
if (sum(str_count(colnames(Tumor_Areas),"grade5")) < 2) { 
  ...
  tumor_grade_labels <- tumor_grade_labels[-1]
  count_tumor_grades_analyzed <- 4
  tumor_grade_colors <- tumor_grade_colors[-1]
} else {
...
Tumor_Areas <- Tumor_Areas %>%
  left_join(select(Slide_Areas, "full_group", full_grouping_vars, "image_id", "slide_number", "individual_id", "subindividual_id"),
    by = c("image_id" = "image_id"),
    suffix = c("", ".y")
  ) %>%
...
Tumor_Areas <- Tumor_Areas %>% mutate(group_grade = paste0(full_group, ":", tumor_grade))
```
...
full_group_tumor_grade_level_cross <-
  expand_grid(full_group_list, c(1:count_tumor_grades_analyzed))
...
full_group_tumor_grade_labels <- setNames(rep(full_group_tumor_grade_labels[[1]], nrow(full_group_labels)), grouping_levels)

group_label_list <- setNames(
  rep(full_group_labels[[1]],
    full_group_count,
    each = count_tumor_grades_analyzed
  ),
  grouping_levels
)
...
tumor_grade_info_list <- setNames(list(setNames(tumor_grade_labels, c(1:count_tumor_grades_analyzed))), "tumor_grade")
...
if (tumor_size_threshold > 0) {
  ...
  filtered_tumor_totals <- Tumor_Areas %>%
    group_by(image_id) %>%
    summarise(
      grade1_percentage_in_slide = sum(grade1_area) / sum(total_tumor_area),
      grade2_percentage_in_slide = sum(grade2_area) / sum(total_tumor_area),
      grade3_percentage_in_slide = sum(grade3_area) / sum(total_tumor_area),
      grade4_percentage_in_slide = sum(grade4_area) / sum(total_tumor_area),
      grade1_area_in_slide = sum(grade1_area),
      grade2_area_in_slide = sum(grade2_area),
      grade3_area_in_slide = sum(grade3_area),
      grade4_area_in_slide = sum(grade4_area),
      total_tumor_area_in_slide = sum(total_tumor_area)
    )
  Slide_Areas$grade1_percentage_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade1_percentage_in_slide
  Slide_Areas$grade2_percentage_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade2_percentage_in_slide
  Slide_Areas$grade3_percentage_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade3_percentage_in_slide
  Slide_Areas$grade4_percentage_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade4_percentage_in_slide
  Slide_Areas$grade1_area_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade1_area_in_slide
  Slide_Areas$grade2_area_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade2_area_in_slide
  Slide_Areas$grade3_area_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade3_area_in_slide
  Slide_Areas$grade4_area_in_slide[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$grade4_area_in_slide
  Slide_Areas$total_tumor_area[match(filtered_tumor_totals$image_id, Slide_Areas$image_id)] <- filtered_tumor_totals$total_tumor_area
} else {
...
if (length(Slide_Areas_unmapped$image_id) > 0 && length(Slide_Areas$image_id) > 0) {
  ...
  kable(select(Slide_Areas_unmapped, "image_id", "animal_id", group_var, "number_of_tumors"), caption = "Ungrouped Slides")
...
if ("exclude_reason" %in% colnames(Slide_Info)) {
  ...
  if (excluded_count > 0) {
    ...
    kable(select(Slide_Areas_excluded, "image_id", "animal_id", group_var, "number_of_tumors", "exclude_reason"), caption = "Excluded slides")
  } else {
...
plot1 <- graph_data %>%
  rowwise() %>%
  mutate(grouping_labels = paste0("<b>",
    full_grouping_vars_labels,
    ": </b>",
    across(all_of(full_grouping_vars)),
    "</br>",
    collapse = ""
  )) %>%
  ggplot(aes(
...
ggplotly_subtitle(plot1, hoverinfo = "text") %>%
  ...
  format_plotly_trace("scatter", "markers+lines",
    hoverinfo = "y",
...
if (!skip_stats) {
  omni_test <- run_omnibus_test(graph_data, graph_formula)
...
stats_summary <- run_stats_summary(graph_data, graph_formula)
if (!skip_stats) {
  omni_test <- run_omnibus_test(graph_data, graph_formula,useParametric = TRUE)
...
plot1 <- graph_data %>%
  mutate(
    tumor_grade = fct_rev(tumor_grade),
    grouping_labels = paste0("<b>",
      full_grouping_vars_labels,
      ": </b>",
      across(all_of(full_grouping_vars)),
      "</br>",
...
graph_data <- Tumor_Areas %>%
  select(full_group, all_of(full_grouping_vars), individual_id, subindividual_id, tumor_grade) %>%
  group_by(full_group, individual_id, tumor_grade) %>%
  mutate(tumor_count_in_individual = n()) %>%
  slice(1) %>%
  group_by(individual_id) %>%
...
stats_summary <- run_stats_summary(graph_data, graph_formula)
if (!skip_stats) {
  omni_test <- run_omnibus_test(stats_summary)
  posthoc_test <- run_posthoc_test(omni_test, full_group_tumor_grade_comparisons)
}
...
plot1 <- graph_data %>%
  ...
  geom_point() + # plot scatter aligned dot plot
  stat_summary(fun = mean, fun.min = min, fun.max = max, shape = 3, size = 2) +
...
ggplotly_subtitle(plot1, hoverinfo = "text") %>%
  ...
  format_plotly_legend("full_group", full_group_info_list, "full_group") %>%
  format_plotly_trace("scatter", "markers+lines", hoverinfo = "y", marker = list(opacity = 0.6))
...
figure_caption <- paste0(
  ...
  paste_to_text_list(c("tumor grade", full_grouping_vars_labels)), "Points/labels in red indicate slidesthat were manually flagged for exclusion (see 'Excluded Slides' table in [Data Filtering]). Data shown in this graph can be found in ",
  file.path(path_to_graph_data, "graph_011_data.csv")
)
```
...
graph_data <- Slide_Areas %>%
  ...
  group_by(individual_id) %>%
  mutate(total_tumor_area_in_individual = sum(total_tumor_area_in_image)) %>%
  slice(1) %>%
  group_by(full_group)
...
figure_caption <- paste0(
  ...
  paste_to_text_list(full_grouping_vars_labels), ". Data shown in this graph can be found in ",
  file.path(path_to_graph_data, "graph_012_data.csv")
)
```
...
plot1 <- graph_data %>%
  rowwise() %>%
  mutate(grouping_labels = paste0("<b>", full_grouping_vars_labels, ": </b>", across(all_of(full_grouping_vars)), "</br>", collapse = "")) %>%
  ggplot(aes(
    x = individual_id,
    y = total_grade_area,
    fill = area_grade,
...
figure_caption <- paste0(
  ...
  ", with bar colors showing the contribution of areas of each tumor grade and bar outlines showing ", full_grouping_label, ". Data shown in this graph can be found in ",
  file.path(path_to_graph_data, "graph_013_data.csv")
)
```
###### Graded tumors
...
graph_data <- Slide_Areas %>%
  ...
  select(
    ...
    -grade3_area_in_image, -grade4_area_in_image, -total_tumor_area_in_image
  ) %>%
  mutate(
    grade1_proportion_in_individual = grade1_area_in_individual / total_tumor_grade_area_in_individual,
    grade2_proportion_in_individual = grade2_area_in_individual / total_tumor_grade_area_in_individual,
...
graph_data <- Tumor_Areas %>%
  select(individual_id, subindividual_id, full_group, all_of(full_grouping_vars), tumor_grade, total_tumor_area) %>%
...
# try to perform stats comparisons between groups
graph_data <- Tumor_Areas %>%
  select(full_group, all_of(full_grouping_vars), individual_id, subindividual_id, tumor_grade, total_tumor_area) %>%
  group_by(individual_id) %>%
  mutate(
...
plot1 <- graph_data %>%
  rowwise() %>%
  mutate(
    grouping_labels = paste0("<b>", full_grouping_vars_labels, ": </b>", across(all_of(full_grouping_vars)), "</br>", collapse = ""),
    tumor_grade = fct_rev(tumor_grade)
  ) %>%
  ggplot(aes(
    x = tumor_grade,
...
figure_legend <- paste0("Graph 020: Box plot of the proportion of total tumor area for each tumor grade within each", paste_to_text_list(full_grouping_vars_labels), ". This does not account for differences in tumor number or tumor burden among the analyzed ", individual_unit)
```
...
graph_data <- Slide_Areas %>%
  select(
    ...
    grade1_area_in_image, grade2_area_in_image, grade3_area_in_image, grade4_area_in_image,
  ...
  select(-subindividual_id, -grade1_area_in_image, -grade2_area_in_image, -grade3_area_in_image, -grade4_area_in_image) %>%
  slice(1)
...
plot1 <- graph_data %>%
  rowwise() %>%
  mutate(grouping_labels = paste0("<b>", full_grouping_vars_labels, ": </b>", across(all_of(full_grouping_vars)), "</br>", collapse = "")) %>%
  pivot_longer(c(grade1_proportion_of_lung_area, grade2_proportion_of_lung_area, grade3_proportion_of_lung_area, grade4_proportion_of_lung_area),
    names_to = "area_grade",
...
graph_data <- Slide_Areas %>%
  select(individual_id, subindividual_id, full_group, all_of(full_grouping_vars), number_of_tumors, total_tumor_area_in_image, total_tissue_area_in_image) %>%
...
figure_caption <- paste0(
  ...
  individual_var_label, 
  ". Data shown in this graph can be found in", 
  file.path(path_to_graph_data, "graph_001_data.csv")
  )
```
...
graph_data <- Slide_Areas %>%
  group_by(individual_id) %>%
  select(individual_id, subindividual_id, full_group, all_of(full_grouping_vars), total_tumor_area_in_image, total_tissue_area_in_image, number_of_tumors) %>%
  mutate(
...
graph_data <- Tumor_Areas %>%
  ...
  mutate(
    total_area_of_graded_tumors = sum(total_tumor_area),
    subindividual_count = n_distinct(subindividual_id),
    graded_tumor_count_in_individual = n()
  ) %>%
  slice(1) %>%
  ...
  group_by(individual_id) %>%
  mutate(
    total_tumor_count_in_individual = sum(graded_tumor_count_in_individual),
    total_tissue_area_in_individual = sum(total_tissue_area_in_image)
  ) %>%
  group_by(individual_id, tumor_grade) %>%
  mutate(total_grade_tumor_proportion_of_lung_area = total_area_of_graded_tumors / total_tissue_area_in_individual) %>%
  select(
    individual_id, full_group,
    all_of(full_grouping_vars),
    tumor_grade,
    subindividual_count,
    graded_tumor_count_in_individual,
    total_area_of_graded_tumors,
...
plot1 <- graph_data %>%
  ...
  ggplot(aes(
    ...
    text = paste0(
      ...
      "<b>Grade ", tumor_grade, " Tumor count: </b>", graded_tumor_count_in_individual, "</br>",
...
graph_data <- Tumor_Areas %>%
  ...
  mutate(
    ...
    graded_tumor_count_in_group = sum(graded_tumor_count_in_individual),
    total_area_of_graded_tumors_in_group = sum(total_area_of_graded_tumors),
    ...
    total_grade_tumor_proportion_of_lung_area_in_group =
      sum(total_grade_tumor_proportion_of_lung_area)
  ) %>%
  slice(1) %>%
  select(
    full_group,
...
graph_data <- Tumor_Areas %>%
  ...
  mutate(
    total_grade_tumor_proportion_of_lung_area =
      total_area_of_graded_tumors / total_tissue_area_in_individual
  ) %>%
  select(
    individual_id,
    full_group,
...
if (!skip_stats) {
  omni_test <- run_omnibus_test(stats_summary,useParametric = FALSE)
  posthoc_test <- run_posthoc_test(omni_test, full_group_comparisons)
}
...
figure_caption <- paste0(
  ...
  " The maximum possible $SDI$ for these comparisons is $ln(4) = 1.386$.",
...
plot1 <- downssampled_graph_data %>% 
  ...
  scale_color_manual(values = group_colors) +
  scale_alpha_continuous(range = c(0.001,0.6),guide=FALSE) +
...
figure_caption <- paste0(
  "Graph 033: Box plot of Shannon's diversity index of individual tumors of each grade from each ",
  paste_to_text_list(full_grouping_vars_labels), 
  ". An $SDI = 0$ indicates only one grade is present within an individual tumor.",
  " The maximum possible $SDI$ for these comparisons is $ln(4) = 1.386$.",
...
figure_caption <- paste0(
  "Graph 034: Scatter plot of Shannon's Diversity Index ($SDI$) against tumor area for individual tumors from each ",
  paste_to_text_list(full_grouping_vars_labels),
  ". An $SDI = 0$ indicates only one grade is present within an individual tumor.",
  " The maximum possible $SDI$ for these comparisons is $ln(4) = 1.386$."
  )
...
figure_caption <- paste0(
  "Graph 035: Dot plot of median Shannon Equitability Index ($SEI$) of individual tumors from each ",
  individual_unit,
...
figure_caption <- paste0(
  "Graph 036: Dot plot of theShannon Equitability Index ($SEI$) of individual tumors from each ",
  full_grouping_label,
  ". An $SEI = 0$ indicates only one grade is present within an individual tumor.",
  "The maximum possible $SEI = 1$ occurs when all grades present occur in equal proportion. ",
  "Groups with > 5000 tumors are randomly sampled."
...
figure_caption <- paste0(
  "Graph 033: Box plot of Shannon's Equitability Index of individual tumors from each ",
  paste_to_text_list(full_grouping_vars_labels), 
  ". An $SEI = 0$ indicates only one grade is present within an individual tumor.",
" The maximum possible $SEI = 1$ occurs when all grades present occur in equal proportion.",
  "Plots show min to max as whiskers with median and IQR shown as boxes.")
...
figure_caption <- paste0(
  "Graph 037: Box plot of Shannon's Equitability Index ($SEI$) of individual tumors of each grade from each ",
  paste_to_text_list(full_grouping_vars_labels),
  ". An $SEI = 0$ indicates only one grade is present within an individual tumor.",
  " The maximum possible $SEI = 1$ occurs when all grades present occur in equal proportion.",
...
figure_caption <- paste0(
  "Graph 038: Scatter plot of Shannon's Equitability Index ($SEI$) against tumor area for individual tumors from each "
  , paste_to_text_list(full_grouping_vars_labels), 
  ". An $SEI = 0$ indicates only one grade is present within an individual tumor.",
  " The maximum possible $SEI = 1$ occurs when all grades present occur in equal proportion."
  )
...
for (fg in present_full_group_labels$label) {
  ...
  mutate(area_grade = factor(area_grade,
    ...
    geom_point(
      ...
      aes(
        x = tumor_uid,
        y = abs(as.numeric(tumor_grade) - count_tumor_grades_analyzed) / count_tumor_grades_analyzed
      )
    ) +
    scale_y_continuous(
...
figure_caption <- paste0(
  "Graph 039: Bar graph(s) of proportion of grades in individual tumors from each ",
  paste_to_text_list(full_grouping_vars_labels),
...
figure_caption <- paste0(
  "Graph 040: Dot plot of number of grades present in individual tumors of each grade from each ",
  paste_to_text_list(full_grouping_vars_labels),
